...
try:
    ...
    arincInPort = session.GetARINCInputPort(0)
    # Only put filter on input port 0, leave port 1 without a filter.

    filterEntry = UeiDaq.tUeiARINCFilterEntry()  # create a new filter entry to put onto the input port.
    filterEntry.Label = 1  # only let through words with an odd label
    filterEntry.NewData = 0
    filterEntry.TriggerSchedulerEntry = 0
    arincInPort.AddFilterEntry(filterEntry)  # add filter entry to port. Also enables filtering for port.

    # When reading a filtered port, we need to make the readers after the filters are added. Use the
    readers = [UeiDaq.CUeiARINCReader(session.GetDataStream()) for i in
               range(inputPorts)]  # create list of readers to read from, in this case its only 1.
